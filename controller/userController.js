...
module.exports = class extends baseController {
  constructor(options) {
    ...
    this.signUp = async (ctx, next) => {
      ...
      const isFind = await this.DBModule.User.findUser({
      ...
      if (isFind.status === "success") {
        if (isFind.data.length > 0) {
          if (isFind.data[0].password === params.password) {
            const isUpdateSuccess = await this.DBModule.User.updateStatus({
              username: params.username,
              status: true
            });
    ...
    this.forceUpdateStatus = async (ctx, next) => {
      const params = ctx.request.body;
      const isUpdateSuccess = await this.DBModule.User.forceUpdateStatus(
        params
      );
      if (isUpdateSuccess) {
        if (isUpdateSuccess.status === "success") {
    ...
    this.addFriend = async (ctx, next) => {
      const params = ctx.request.body;
      const isFindUser = await this.DBModule.User.findUser({
        username: params.username
      });
      const isFind = await this.DBModule.User.findUser({
      ...
      if (isFind.status === "success") {
        if (isFind.data.length > 0) {
          ...
          if (haveThis) {
            ctx.body = { status: 400, msg: "添加失败，提示该用户已是你的好友" };
          } else {
            const isAddSuccess = await this.DBModule.User.addFriend({
              username: params.username,
              friend: params.friend
            });
            const isAddMe = await this.DBModule.User.addFriend({
              username: params.friend,
              friend: params.username
            });
            if (
              isAddSuccess.status === "success" &&
              isAddMe.status === "success"
            ) {
              ctx.body = {
                status: 200,
                msg: "添加成功",
                data: isAddSuccess.data
              };
            } else {
