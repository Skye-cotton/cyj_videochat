...
const fs = require("fs");
let sslOptions = {
  key: fs.readFileSync("F:/ssl-key/privkey.key"), // 里面的文件替换成你生成的私钥
  cert: fs.readFileSync("F:/ssl-key/cacert.pem") // 里面的文件替换成你生成的证书
};
...
io.on("connection", function(socket) {
  ...
  socket.on("receive", function(msg, from, to) {
    ...
    if (socketId) {
      io.sockets.sockets[socketId].emit("newMsg", {
        from: from,
        to: to,
        msg: msg,
        date: date
  ...
  socket.on("disconnect", async function() {
    ...
    const result = await DBModule.User.updateStatus({
      username: username,
      status: false
    });
    console.log(result);
